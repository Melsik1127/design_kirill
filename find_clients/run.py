...
def data(call):
    if call.message:
        if call.data == "accept_bell":
            ...
            bot.send_voice(call.message.chat.id, open("audio/bell.ogg", "rb"))
...
Жми кнопку “Продолжить” и следуй указателям 👇🏻""".format(user=user_name), reply_markup=markup)
        ...
        if call.data == "find_client":
            ...
            bot.send_photo(chat_id=call.message.chat.id, photo=open("img/9.jpg","rb"), caption = """
<b>🔋 Все просто</b>

Напиши в строку любое ключевое слово
...
Например:
    ...
    Нужен сайт
    Нужен таргетолог
    Ищу кто сделает сайт
    И так далее...
    
💸 Тебе покажутся самые свежие объявления, остается всего лишь ответить на его и предложить свою помощь            
...
👾 Давай исправим это!""", reply_markup=markup)
            if result == "1":
                ...
                butt_3 = InlineKeyboardButton("ℹ️ Тех.поддержка", callback_data="help")
                markup.add(butt_1, butt_2, butt_3)

                bot.send_video(call.message.chat.id, open("img/gifs/5.gif", "rb"), caption="""
🎉 Супер! <b>Твоя подписка активна</b>

✅ Функция поиска клиента разблокирована

Можешь спокойно приступить к поиску""", reply_markup=markup, parse_mode="html")
